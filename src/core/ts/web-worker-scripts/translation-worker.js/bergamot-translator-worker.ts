...
function ___sys_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(4, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___sys_open(path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(6, 1, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
...
function ___sys_unlink(path) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(9, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.unlink(path);
  return 0;
...
var proxiedFunctionTable = [ null, _atexit, ___sys_access, ___sys_fcntl64, ___sys_getcwd, ___sys_ioctl, ___sys_open, ___sys_rmdir, ___sys_stat64, ___sys_unlink, _emscripten_set_canvas_element_size_main_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write, _tzset ];
