...
describe("Translation: Simultaneous tabs with different language pairs", function() {
  ...
  it("Telemetry checks after: Translation via the infobar works when translating in multiple tabs with different language pairs at the same time", async function() {
    // ... this test continues the session from the previous test
    const seenTelemetry = await readSeenTelemetry(0, 3, proxyInstanceId);
    assertOnTranslateButtonPressedTelemetry(seenTelemetry[0], "es", "en");
    assertOnTranslateButtonPressedTelemetry(seenTelemetry[1], "et", "en");
    assertOnTranslationAttemptConcludedTelemetry(seenTelemetry[2], "es", "en");
    assertOnTranslationAttemptConcludedTelemetry(seenTelemetry[3], "et", "en");
  });
});
